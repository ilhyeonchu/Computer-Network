...
# Raspberry Pi 설정
RASPI_HOST = "dnlab@192.168.1.42"
RASPI_PROJECT_DIR = "/home/dnlab/mcp_project"
...
def status():
    ...
    if not test["success"] or "connected" not in test["stdout"]:
        return jsonify({
            "connected": False,
            "error": "Raspberry Pi에 연결할 수 없습니다."
        })
    ...
    info = {
        ...
        "memory": run_ssh_command("free -h | grep Mem | awk '{print $3\"/\"$2}'")["stdout"].strip(),
        "disk": run_ssh_command("df -h / | tail -1 | awk '{print $3\"/\"$2\" (\"$5\")\"}'")["stdout"].strip(),
