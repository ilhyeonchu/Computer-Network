...
async def fetch_it_news(max_count: int = 50) -> str:
    """네이버 IT 섹션 뉴스 제목을 가져옵니다."""
    texts = fetch_news_texts(DEFAULT_SECTION_URLS)
    if not texts:
        return "⚠️ 뉴스 텍스트를 찾지 못했습니다."
    return "\n".join(texts[:max_count])
...
def main_cli():
    import argparse
    p = argparse.ArgumentParser(description="Naver IT/과학 섹션 크롤러")
    p.add_argument("--mode", choices=["print", "csv", "wc"], default="print",
                   help="print: 제목 출력, csv: CSV 저장, wc: 워드클라우드")
    p.add_argument("--url", default=DEFAULT_SECTION_URL,
